...
void ShaderProgram::load(const char *vertex_shader_file, const char *fragment_shader_file) {
    ...
    if (m_program_id != 0) {
           // Detach and delete existing shaders and program
    ...
    m_greyscale_flag            = glGetUniformLocation(m_program_id, "greyscaleFlag");
...
//
////used to switch shader style when key is pressed
void ShaderProgram::set_greyscale_flag(GLuint flag)
{
    glUseProgram(m_program_id);
    glUniform1i(m_greyscale_flag, flag);
}
